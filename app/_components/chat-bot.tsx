...
const ChatBot = ({
  ...
  const sendMessages = async (messages:MessageAI[]) => {
    try {
      ...
      appendMessage("assistant", "My brain went offline for a sec — classic ‘AI brain fog.’ Trying to reboot my wisdom!");
  ...
  const handleToolCalls = async (toolCalls: ToolCall[]) => {
    ...
    for (const toolCall of toolCalls) {
      if (toolCall.function.name === 'celebrateSuccess') {
        celebrateSuccess();
        toolMessages.push({role:'tool', content:'Success message displayed.', tool_call_id:toolCall.id});
      } else {
        const result = await functionCallHandler(getNow(), getTimeZone(), toolCall);
  ...
  */
  ...
  const appendMessage = (role: "user" | "assistant" | "success", text: string) => {
